 ...
 * Title:        arm_batch_norm_f32.c
 * Description:  Batch normalization for float32 tensors
 *
 * $Date:        19 March 2026
 * $Revision:    V.1.0.0
 *
    ...
    for (int32_t bn = 0; bn < n; ++bn)
    {
        for (int32_t y = 0; y < h; ++y)
        {
            for (int32_t x = 0; x < w; ++x)
            {
                ...
                for (int32_t ch = 0; ch < c; ch += 4)
                {
                    const mve_pred16_t p = vctp32q((uint32_t)(c - ch));
                    const float32x4_t v_in = vld1q_z(input + base + (size_t)ch, p);
                    const float32x4_t v_scale = vld1q_z(scale + ch, p);
                    const float32x4_t v_bias = vld1q_z(bias + ch, p);
                    const float32x4_t v_out = vfmaq(v_bias, v_in, v_scale);
                    vst1q_p(output + base + (size_t)ch, v_out, p);
                }
    #else
                for (int32_t ch = 0; ch < c; ++ch)
                {
                    output[base + (size_t)ch] = input[base + (size_t)ch] * scale[ch] + bias[ch];
                }
    #endif
            }
        }
