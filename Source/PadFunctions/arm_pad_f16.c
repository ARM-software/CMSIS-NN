 ...
 * Title:        arm_pad_f16.c
 * Description:  Pad a float16 tensor
 *
 * $Date:        19 March 2026
 * $Revision:    V.1.0.0
 *
                                ...
                                const cmsis_nn_dims *post_pad)
{
    ...
    arm_memset_f16(output, pad_value, batch_block_size * (uint32_t)pre_pad->n);
    output += batch_block_size * (uint32_t)pre_pad->n;
    for (int32_t b = 0; b < input_size->n; ++b)
    {
        arm_memset_f16(output, pad_value, row_block_size * (uint32_t)pre_pad->h);
        output += row_block_size * (uint32_t)pre_pad->h;
        for (int32_t y = 0; y < input_size->h; ++y)
        {
            arm_memset_f16(output, pad_value, col_block_size * (uint32_t)pre_pad->w);
            output += col_block_size * (uint32_t)pre_pad->w;
            if (input_size->c == output_size.c)
            {
                arm_memcpy_f16(output, input, (uint32_t)input_size->w * (uint32_t)input_size->c);
            ...
            else
            {
                ...
                {
                    arm_memset_f16(output, pad_value, (uint32_t)pre_pad->c);
                    output += pre_pad->c;

                    arm_memcpy_f16(output, input, (uint32_t)input_size->c);
                    output += input_size->c;
                    input += input_size->c;

                    arm_memset_f16(output, pad_value, (uint32_t)post_pad->c);
                    output += post_pad->c;
                }
            }

            arm_memset_f16(output, pad_value, col_block_size * (uint32_t)post_pad->w);
            output += col_block_size * (uint32_t)post_pad->w;
        }

        arm_memset_f16(output, pad_value, row_block_size * (uint32_t)post_pad->h);
        output += row_block_size * (uint32_t)post_pad->h;
    }
    arm_memset_f16(output, pad_value, batch_block_size * (uint32_t)post_pad->n);
