 ...
 * Title:        arm_py_fully_connected_buffer_size.cpp
 * Description:  Fully connected buffer size pybinds (optional Python module)
 *
 * $Date:        5 Mar 2026
 * $Revision:    V.1.0.0
 *
    ...
    m.def(
        ...
        [](Backend backend, DataType data_type, const std::array<int32_t, 4> &filter_nhwc) -> int32_t {
            ...
            case DataType::A16W8:
                ...
                {
                case Backend::MVE:
                    return arm_fully_connected_s16_get_buffer_size_mve(&filter_dims);
                case Backend::DSP:
                    return arm_fully_connected_s16_get_buffer_size_dsp(&filter_dims);
                case Backend::SCALAR:
