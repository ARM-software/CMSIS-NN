...
if ARGS.interpreter == 'tensorflow':
...
class TestSettings(ABC):
    ...
    def __init__(self, dataset, testtype, args, in_ch, out_ch, x_in, y_in, w_x, w_y, stride_x=1, stride_y=1, pad=False,
        ...
        self.tensor_flow_reference_version = ("// Generated by {} using tensorflow version {} (Keras version {}).\n".
                                              format(os.path.basename(__file__), tf.__version__, tf.keras.__version__))
        ...
        self.tensor_flow_reference_version += ("// Interpreter from {} version {} and revision {}.\n".
                                               format(interpreter, version, revision))
        ...
        def representative_data_gen():
            ...
            if n_inputs > 0:
                for i in range(n_inputs):
                    representative_testsets.append(np.ones(shape,
                                                           dtype=np.float32))
...
class LSTMSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype, args, batches=2, time_steps=2, number_inputs=3,
                 number_units=4, time_major=True, randmin=INT8_MIN, randmax=INT8_MAX, generate_bias=True):
        super().__init__(dataset, testtype, args, 1, 1, 1, 1, 1, 1, 1, 1, False, randmin,
                         randmax, generate_bias=generate_bias)
    ...
    def generate_data(self, input_data=None, weights=None, hidden_weights = None, biases=None) -> None:
        ...
        input_layer = tf.keras.layers.Input(shape=(self.time_steps, self.number_inputs),
                                            batch_size=self.batches, name='input')
        if self.time_major:
            input_layer_transposed = tf.transpose(input_layer, perm=[1,0,2])
            lstm_layer = tf.keras.layers.LSTM(units=self.number_units, time_major=self.time_major,
        ...
        interpreter = self.convert_and_interpret(
            model, tf.int8, input_data, dataset_shape=shape)
        ...
        recurrent_to_forget_eff_bias = self.calc_effective_bias(interpreter,
                                                                output_state_zp,
                                                                recurrent_input_to_forget_w,
                                                                None, False)
        input_to_cell_eff_bias = self.calc_effective_bias(interpreter,
                                                          input_zp,
                                                          input_to_cell_w,
                                                          cell_gate_bias)
        recurrent_to_cell_eff_bias = self.calc_effective_bias(interpreter,
                                                              output_state_zp,
                                                              recurrent_input_to_cell_w,
                                                              None, False)
        input_to_output_eff_bias = self.calc_effective_bias(interpreter,
                                                                  input_zp,
                                                                  input_to_output_w, output_gate_bias)
        recurrent_to_output_eff_bias = self.calc_effective_bias(interpreter,
                                                                output_state_zp,
                                                                recurrent_input_to_output_w,
                                                                None, False)
        input_to_input_eff_bias = self.calc_effective_bias(interpreter, input_zp, input_to_input_w, input_gate_bias)

        recurrent_to_input_eff_bias = self.calc_effective_bias(interpreter,
                                                               output_state_zp,
                                                               recurrent_input_to_input_w,
                                                               None, False)
...
def load_testdata_sets() -> dict:
    ...
    testdata_sets[dataset] = ConvSettings(dataset, type_of_test, args, in_ch=8, out_ch=nbr_of_out_channels, x_in=4,
                                          y_in=4, w_x=2, w_y=2, stride_x=1, stride_y=1, pad=False, randmin=INT16_MIN,
                                          randmax=INT16_MAX, out_activation_min=-17000, out_activation_max=32767,
                                          int16xint8=True, generate_bias=bias)
    ...
    type_of_test = 'lstm'
    dataset = 'lstm_1'
    testdata_sets[dataset] = LSTMSettings(dataset, type_of_test, args, batches=1, time_steps=10,
                                          number_inputs=5, number_units=3, time_major=True)
    dataset = 'lstm_2'
    testdata_sets[dataset] = LSTMSettings(dataset, type_of_test, args, batches=2, time_steps=9,
                                          number_inputs=6, number_units=5, time_major=False)
    dataset = 'lstm_one_time_step'
    testdata_sets[dataset] = LSTMSettings(dataset, type_of_test, args, batches=2, time_steps=1,
                                          number_inputs=4, number_units=4, time_major=False)
...
if __name__ == '__main__':
    ...
    if args.run_all_testsets:
        ...
        try:
            ...
            elif args.testtype == 'lstm':
                generator = LSTMSettings(testdataset, test_type, args)
            else:
